...
fn main() -> ! {
    ...
    // Enable LEDC clock.
    ccu.ledc_clk
        .write(|w| w.clk_gating().on().clk_src_sel().hosc());
    ...
    ccu.pll_video0_ctrl
        .modify(|_, w| w.pll_output_gate().set_bit());
    ...
    lcd0.lcd_ctl_reg.write(|w| unsafe {
        w.bits(
    ...
    lcd0.lcd_basic2_reg.write(|w| unsafe {
        w.bits(
            // LCD_BASIC2_REG.VT=VT_COUNT for VT_COUNT/2 vertical rows total
            (((RES_Y + 21)*2) << 16)
